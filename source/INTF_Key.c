...
volatile static uint8 g_CredFlg = CREDFLG_DIS;	/* 是否发送不可靠按键的标志位 */
...
{
	if (KEY_PRESSING == PORT_GetBit(KEYPTT_PORT, KEYPTT_PIN))	/* 按键按下 */
	{
		return 1;
	}
	if (KEY_PRESSING == PORT_GetBit(KEYPTT_E_PORT, KEYPTT_E_PIN))	/* 按键按下 */
	{
		return 1;
	}
...
uint8 GetKeypad(void)
{
	...
	for(j=0;j<4;j++)
	{
		...
		{
			KeyChkLock();//公用	
			keypad_pn_setallhigh();//asm("NOP");asm("NOP");//置高
			if (KEY_PRESSING == GPIO_ReadInputDataBit((GPIO_TypeDef* )KEYPAD_INT_GPIOGRP[j], KEYPAD_INT_PINGRP[j]))
			{
				Stat =	keypad_arrP[j];
			}
...
uint8 KeyStatChk(void)
{
	...
	//*perr = 0;	/* 默认ok，否则配置为非0表示未获取到按键 */
//	if (Reset == PORT_GetBit(KEYPTT_PORT, KEYPTT_PIN))	/* 按键按下 */
//	{
//		Delay_ms(10);
...
//	if (Reset == PORT_GetBit(KEYPTT_E_PORT, KEYPTT_E_PIN))	/* 按键按下 */
//	{
//		Delay_ms(10);
...
//	{//先按住PTT，再按住自定义按键,后松开PTT按键，还在发射的问题.
//		//此处检测到的是PTT松开的动作.
//		Stat = 0;
//		return Stat;
//	}
...
/*****************************************************************************
 *	函数名:KeyHandle
 *	 函数功能: 按键处理函数，若有相应按键触发，则执行
 *	对应操作
 *   创建人：陶林，2009/10/13
******************************************************************************/
void KEY_INTF_KeyScan (void) // 按键中断后的按键查询处理。
{
//	uint8 err;
//	uint8 ErrGetKey;	/* 获取按键值的返回错误参数 */
	uint8 CurKeyStat;	/* 临时保存按键值 */
...
uint32 KeyGetOpt(uint32 TimeOut)
{
	...
	g_CredFlg = CREDFLG_EN;	/* 使能能接受不可靠按键 */
 ...
 *   函数名: KeyPutOpt
 *   功能描述: 输出一个按键
 *   参数定义: 	Flg:KEY_CRED:可靠按键，否则为不可靠按键
 *					Stat:输出按键值
 *	返回值:
 *   创建人：陶林，2009/10/13
******************************************************************************/
