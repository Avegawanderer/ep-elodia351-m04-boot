    ...
    repeat:
        ...
        switch (*fmt)
        {
            ...
            case 'p':
                ...
                str = ck_number(str, end, (unsigned long)ck_va_arg(args, void *),
        ...
        str = ck_number(str, end, num, base, field_width, precision, flags);
    }
    if (str <= end)
        *str = '\0';
    else if (size > 0)
...
static char *
ck_number(char *buf, char *end, long num, int base, int size, int precision,
          int type)
{
    char c, sign, tmp[66];
    const char *digits;
